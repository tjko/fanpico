...
static int read_register(tmp117_context_t *ctx, uint8_t reg, uint16_t *val)
{
	uint8_t buf[2];
	int res;

	res = i2c_write_blocking(ctx->i2c, ctx->addr, &reg, 1, true);
	if (res < 1)
		return -1;

	res = i2c_read_blocking(ctx->i2c, ctx->addr, buf, 2, false);
	if (res < 2)
		return -2;
...
static int write_register(tmp117_context_t *ctx, uint8_t reg, uint16_t val)
{
	uint8_t buf[3];
	int res;
	...
	res = i2c_write_blocking(ctx->i2c, ctx->addr, buf, 3, false);
	if (res < 1)
		return -1;
...
void* tmp117_init(i2c_inst_t *i2c, uint8_t addr)
{
	...
	res  = read_register(ctx, TMP117_REG_DEVICE_ID, &val);
	if (res || val != TMP117_DEVICE_ID)
		goto panic;

	/* Reset Sensor */
	res = write_register(ctx, TMP117_REG_CONFIG, 0x0222);
	if (res)
		goto panic;
	...
	res = read_register(ctx, TMP117_REG_CONFIG, &val);
	if (res)
		goto panic;
...
int tmp117_get_measurement(void *ctx, float *temp)
{
	...
	/* Get Measurement */
	res = read_register(ctx, TMP117_REG_TEMP_RESULT, &val);
	if (res)
		return -2;

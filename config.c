...
void save_config()
{
	...
	for (i = 0; i < FAN_MAX_COUNT; i++) {
		...
		cJSON_AddItemToObject(o, "s_type", cJSON_CreateString(pwm_source2str(f->s_type)));
		cJSON_AddItemToObject(o, "s_id", cJSON_CreateNumber(f->s_id));
	...
	for (i = 0; i < MBFAN_MAX_COUNT; i++) {
		...
		cJSON_AddItemToObject(o, "s_type", cJSON_CreateString(tacho_source2str(m->s_type)));
		cJSON_AddItemToObject(o, "s_id", cJSON_CreateNumber(m->s_id));
...
panic:
	printf("save_config(): Out of memory?");
	if (config)
		cJSON_Delete(config);
}
